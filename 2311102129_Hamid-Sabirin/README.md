...
dependencies:
  flutter:
    sdk: flutter
  image_picker: ^1.1.2 # Plugin untuk memanggil API Kamera & Galeri bawaan OS
  flutter_local_notifications: ^17.1.2 # Plugin untuk membuat notifikasi pop-up sistem
```
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android">
    <!-- Izin agar notifikasi bisa di-trigger bahkan jika HP baru menyala -->
    <uses-permission android:name="android.permission.RECEIVE_BOOT_COMPLETED"/>
    <!-- Izin untuk menggunakan fitur getar (Vibration) saat notifikasi masuk -->
    <uses-permission android:name="android.permission.VIBRATE" />
    <!-- Izin untuk membangunkan layar HP saat notifikasi masuk -->
...
```dart
  Future<void> _getImage(ImageSource source) async {
    ...
    if (pickedFile != null) {
      ...
      // 4. Memainkan animasi transisi agar gambar tidak muncul secara kaku
      _animationController?.reset();
      _animationController?.forward();
    
      // 5. Secara paralel, panggil fungsi _showNotification untuk memberi tahu pengguna
...
**Penjelasan:**
Penggunaan `await _picker.pickImage()` sangat penting agar aplikasi Flutter tidak *freeze* atau *hang* saat antarmuka kamera HP terbuka. Penggunaan `setState()` juga menjadi kunci mengapa gambar yang awalnya berupa *placeholder* kosong tiba-tiba bisa terisi dengan gambar asli. Tanpa `setState`, meskipun variabel `_image` sudah berisi data foto, layar tidak akan menyadarinya.
...
```dart
  Future<void> _showNotification(String source) async {
    ...
    const AndroidNotificationDetails androidNotificationDetails =
      ...
      'photo_channel', // ID unik untuk channel notifikasi (wajib untuk Android 8.0+)
      'Photo Notifications', // Nama channel yang terlihat di pengaturan HP
      ...
      ticker: 'ticker', // Teks lama yang biasa muncul sesaat di status bar
    );
    ...
    await flutterLocalNotificationsPlugin.show(
      id: 0, // ID notifikasi (jika ID-nya sama, notifikasi sebelumnya akan tertimpa)
      title: '✨ Foto Berhasil!', // Judul huruf tebal pada notifikasi
      body: 'Foto berhasil didapatkan dari $source', // Isi teks notifikasi
      notificationDetails: notificationDetails, // Memasukkan konfigurasi dari step 2
    );
  }
```

**Penjelasan:**
Pada Android versi 8.0 (Oreo) ke atas, semua notifikasi wajib dimasukkan ke dalam sebuah **Channel**. `AndroidNotificationDetails` menjamin bahwa notifikasi ini memiliki *channel* bernama `photo_channel`. Parameter `Importance.max` dan `Priority.high` adalah trik pemrograman agar notifikasi tidak hanya bersembunyi secara diam-diam di menu *tray*, melainkan muncul secara proaktif dan jatuh dari atas layar secara agresif (yang sering disebut *Heads-up Notification*).
